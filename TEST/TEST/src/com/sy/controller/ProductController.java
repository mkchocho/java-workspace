...
import com.sy.model.dao.ProductDao;
...
public class ProductController {
	...
	public void insertProduct(String productId,String pName,String price,String description,String stock) {
		...
		if(result > 0) {
			new MainMenu().displaySuccess("상품이 성공적으로 추가되었습니다.");
		}else {         
			new MainMenu().displayFail("상품 추가에 실패했습니다.");
		}
	}
	
	 //상품전체를 조회요청을 처리하는 메서드
	 
	public void selectList() {
		ArrayList<Product> list = new ProductDao().selectList();
		
		if(list.isEmpty()) {
			new MainMenu().displayNoData("조회된 결과가 없습니다.");
		}else {
	...
	public void updateProduct(String productId,String price,String description) {
		...
		if(result > 0 ) { //성공
			new MainMenu().displaySuccess("상품 정보가 성공적으로 변경되었습니다.");
		}else { //실패
			new MainMenu().displayFail("상품 정보 변경에 실패했습니다.");
		}
	}
