 ...
 */
	...
	public void selectByName(String userName) {
		...
		if(list.isEmpty()) {
			new MainMenu().displayNoData(userName+"(이)라는 회원이 없습니다.");
		} else {
			new MainMenu().displayMemberList(list);
		}
	 ...
	 */
	public void updateMember(String userId, String usrePwd, String email, String phone, String address) {
		Member m = new Member();
		m.setUserId(userId);
		m.setUserPwd(usrePwd);
		m.setEmail(email);
		m.setPhone(phone);
	...
	public void deleteMember(String userId) {
		...
		if(result >0) { //성공
			...
			new MainMenu().displayFail("회원정보 삭제에 실패했습니다.");
		}
	}
